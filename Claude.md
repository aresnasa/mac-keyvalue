...
38. 使用 gh 命令创建 github 项目，然后打包 dmg 并发布，本项目使用 mit 协议，调整并发布到 github和 brew
39. 检查图标，应该是一个带锁的信封而不是一个黑色的信封这样不太好。改造下图标和 github 上的图标，K和V 中间使用锁进行链接，象征数据的加密，不要用黑色的信封。这里可以为锁增加一些简单的特效。
